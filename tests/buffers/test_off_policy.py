""" Tests for Off Policy Replay Buffer """
...
def replay_buffer(buffer_size):
    ...
    return OffPolicyBuffer(buffer_size=buffer_size, epoch_size=buffer_size)
...
def test_finish_path_smoke(replay_buffer):
    ...
    for _ in range(5):
        replay_buffer.store(to_buffer)
    replay_buffer.finish_path(last_obs=np.array([4, 5, 6]))
...
def test_finish_path_1d_obs(replay_buffer):
    ...
    for _ in range(5):
        replay_buffer.store(to_buffer)
    replay_buffer.finish_path(last_obs=np.array([4]))
    # ensure next_obs was set properly
    np.testing.assert_equal(
        replay_buffer.buf['obs'][1], replay_buffer.buf['next_obs'][0])
    np.testing.assert_equal(
...
def test_raises_notimp_if_buffer_not_multiple_of_epoch_size():
    ...
    with pytest.raises(NotImplementedError):
        _ = OffPolicyBuffer(buffer_size=3, epoch_size=2)
...
def test_sample_batch(replay_buffer, to_store):
    ...
    replay_buffer.finish_path(last_obs=to_store['obs'])
